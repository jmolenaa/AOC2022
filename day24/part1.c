...
{
	if (abs(map->x_end - map->x_expedition) + abs(map->y_end - map->y_expedition) > minsteps - steps)
		return (1);
	return (0);
}
...
void	change_position(t_map *map, int steps, int *minsteps, t_location **locations)
{
	if (check_locations_and_add(map, steps, locations) || check_manhattan(map, steps, *minsteps) || steps >= *minsteps)
		return ;
	if (map->x_expedition == map->x_end && map->y_expedition == map->y_end - 1)
	{
		if (steps < *minsteps)
			*minsteps = steps;
		return ;
...
void	move(t_map *map)
{
	...
	change_position(map, steps, &minsteps, &locations);
