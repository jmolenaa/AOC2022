...
void	reset_chamber(t_chamber *chamber)
{
	int	i = 0;
	while (i < 200)
	{
		// printf("%d\n", chamber->currentheight);
		ft_strcpynonull(*(chamber->chamber + chamber->ymax - 199 + i),\
		 *(chamber->chamber + chamber->currentheight + i));
		i++;
	}
	while (i < chamber->ymax)
	{
		ft_strcpynonull(*(chamber->chamber + chamber->ymax - i), "@.......@");
		i++;
	}
}
...
void	let_rocks_fall(t_jet *jet, t_rocks *rocks, t_chamber *chamber)
{
	...
	for (int j = 0; j < 1000000; j++)
	{
		...
		reset_chamber(chamber);
		//at some point this(v) starts repeating, put in file, check from which point it repeats\
		and start counting lol
		if (actual_height == 0)
		{
			printf("%d : %lld, diff\n", j, actual_height + (chamber->ymax - chamber->currentheight) - actual_height);
			actual_height = chamber->ymax - chamber->currentheight;
		}
		else
		{
			actual_height = actual_height + (chamber->ymax - chamber->currentheight - 199);
			printf("%d : %lld, diff\n", j, actual_height + (chamber->ymax - chamber->currentheight - 199) - actual_height);
		}
		chamber->currentheight = chamber->ymax - 199;
	}
	printf("rock height\n%lld\n", actual_height);
}
